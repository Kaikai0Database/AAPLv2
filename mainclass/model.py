...
class ProteinDataset(Dataset):
    ...
    def __init__(self, df, batch_converter, padding, paddingNumber):
        self.df = df
...
class multiClassifier(nn.Module):
    ...
    def __init__(self, model_type, input_dim, num_labels, num_layers=2, kernel_size=(3, 3), dropout=0.6):
        super(multiClassifier, self).__init__()
        self.model_type = model_type

        if self.model_type == "LSTM":
            self.lstm1 = nn.LSTM(input_size=input_dim, hidden_size=16, bidirectional=True,
            ...
            self.conv1 = nn.Conv1d(in_channels=input_dim, out_channels=16, kernel_size=3, stride=2)
            ...
            self.conv1 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=kernel_size, padding='same')
        ...
        elif self.model_type == "CNN2D+LSTM":
            ...
            self.pool = nn.MaxPool2d(kernel_size=(2, 2))
            cnn_out_width = input_dim // 2
            self.lstm_input_features = 128 * cnn_out_width
            self.feature_proj = nn.Linear(self.lstm_input_features, 256)
            self.relu = nn.ReLU()
            self.lstm = nn.LSTM(input_size=256, hidden_size=64, num_layers=num_layers, batch_first=True)
        ...
        elif self.model_type == "CNN2D+Linear":
            self.conv1 = nn.Conv2d(in_channels=1, out_channels=64, kernel_size=(3, 3), padding='same')
            self.bn1 = nn.BatchNorm2d(64)
        ...
        if "CNN1D+Linear" in self.model_type:
            self.global_pool = nn.AdaptiveMaxPool1d(1)
            self.flatten = nn.Flatten()
            self.sigmoid = nn.Sigmoid()
            if self.model_type == "CNN1D+Linear":
                self.conv1 = nn.Conv1d(in_channels=input_dim, out_channels=128, kernel_size=3, padding='same')
                ...
                self.fc2 = nn.Linear(64, num_labels)
            elif self.model_type == "CNN1D+Linear_NoConv2":
                self.conv1 = nn.Conv1d(in_channels=input_dim, out_channels=128, kernel_size=3, padding='same')
                self.bn1 = nn.BatchNorm1d(128)
                self.fc1 = nn.Linear(128, 64)
                self.bn_fc = nn.BatchNorm1d(64)
                self.dropout = nn.Dropout(0.3)
                self.fc2 = nn.Linear(64, num_labels)
            elif self.model_type == "CNN1D+Linear_NoFC1":
                self.conv1 = nn.Conv1d(in_channels=input_dim, out_channels=128, kernel_size=3, padding='same')
                self.bn1 = nn.BatchNorm1d(128)
                self.conv2 = nn.Conv1d(in_channels=128, out_channels=256, kernel_size=3, padding='same')
                self.bn2 = nn.BatchNorm1d(256)
                self.fc2 = nn.Linear(256, num_labels)
            elif self.model_type == "CNN1D+Linear_NoBN":
                self.conv1 = nn.Conv1d(in_channels=input_dim, out_channels=128, kernel_size=3, padding='same')
                self.conv2 = nn.Conv1d(in_channels=128, out_channels=256, kernel_size=3, padding='same')
                self.fc1 = nn.Linear(256, 64)
                self.dropout = nn.Dropout(0.3)
    ...
    def forward(self, x, return_features=False):
        ...
        if self.model_type == "LSTM":
            ...
            lstm_out = self.dropout(self.tanh(self.bn_lstm(lstm_out[:, -1, :])))
        ...
        if "CNN1D+Linear" in self.model_type:
            ...
            if self.model_type == "CNN1D+Linear":
                ...
                if return_features: features['conv1_out'] = x.clone().detach()
                x = torch.relu(self.bn2(self.conv2(x)))
                if return_features: features['conv2_out'] = x.clone().detach()
                x = self.dropout(torch.relu(self.bn_fc(self.fc1(self.flatten(self.global_pool(x))))))
                if return_features: features['fc1_out'] = x.clone().detach()
                logits = self.fc2(x)
            elif self.model_type == "CNN1D+Linear_NoConv2":
                x = torch.relu(self.bn1(self.conv1(x)))
                if return_features: features['conv1_out'] = x.clone().detach()
